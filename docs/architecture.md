...
Protocol v3 registrations contain workspace folders and terminal PID/cwd metadata plus a unique token for every terminal. Ordinary terminals receive a Bridge-assigned token that is persisted for the terminal process lifetime. On `SessionStart` or `UserPromptSubmit`, the Hook claims the compatible active terminal in the focused VS Code window and persists its token and PID in session state. Managed terminals additionally inherit `CCMONITOR_TERMINAL_TOKEN` from their environment, but managed creation is optional.

The desktop app selects by an explicit session-terminal binding first, then terminal token, then PID, then safe cwd/project matching. It writes a request containing `targetBridgeId`; only that bridge answers. cwd/workspace is primarily a bootstrap and recovery signal, while steady-state focusing uses token/PID. Users can create an explicit binding with `CC Monitor: Bind Active Terminal to Session` only when automatic claiming is ambiguous. Results explicitly report `matched`, `noMatch`, or, from the app when no heartbeat exists, `bridgeNotRunning`.
