...
- 每个 VS Code 窗口由 Terminal Bridge 主动登记 workspace、终端 PID、cwd、稳定 token 和当前活动终端；每个普通 terminal 也会自动获得唯一 Bridge token，CC Monitor 不再依赖 Hook 进程树猜测 terminal PID。
- 普通 terminal 直接运行 `claude` 即可。首次 `SessionStart` 或 `UserPromptSubmit` 会自动认领聚焦窗口中的活动 terminal，并把它的 token 和 PID 保存到 session；不要求使用 Managed Terminal 或手动 Bind。
...
1. **自动 Bridge token**：扩展为所有 terminal 自动分配唯一 token；用户在普通 terminal 中启动 Claude 后，Hook 会自动认领当前活动 terminal。
2. **Managed 环境 token**：可选执行 **CC Monitor: Create Managed Claude Terminal**；环境 token 可在 Claude 启动时直接继承，但不是正常使用的前置条件。
3. **手动绑定兜底**：只有自动认领无法唯一确定时，才在 session 菜单选择 **Bind terminal…**，然后执行 **CC Monitor: Bind Active Terminal to Session**。
4. **安全 cwd/workspace 引导**：首次认领时使用 terminal cwd 或 workspace 约束候选；长期切换使用已保存的 token/PID。
...
可从 [Gitea Releases](https://gitea.lan.fasteurai.com/linruyue/claude-code-monitor-desktop/releases) 或 [GitHub Releases](https://github.com/LornaCc/claude-code-monitor/releases) 下载 `CCMonitor-v0.5.2-win-x64.zip`。
...
5. 在任意 VS Code integrated terminal 中正常运行 `claude`；Managed Terminal 和手动 Bind 都不是必需步骤。

安装器会停止所有旧 CC Monitor 实例，将 v0.5.2 安装到 `%LOCALAPPDATA%\Programs\CCMonitor\0.5.2`，重新指向 Claude Code Hooks 和 StatusLine，强制安装并核对 Terminal Bridge 0.5.2，更新开始菜单和桌面快捷方式，然后只启动新版本。旧版本目录可以保留用于兼容尚未重载的 Claude Code 进程，但安装器管理的 Hook、快捷方式和运行进程都会指向新版。
...
- **Terminal Bridge is not running**：确认安装了发布包内的最新扩展，并在每个相关 VS Code 窗口执行 **Developer: Reload Window**。
- **Terminal not found**：先激活目标 terminal 并在其中提交一次 prompt，让 session 自动认领；仍无法判断时再使用 **Bind terminal…**。
- **同一 cwd 有多个 terminal**：`SessionStart`/`UserPromptSubmit` 会认领聚焦窗口中的活动 terminal；只有无法唯一确定时才需要手动绑定，CC Monitor 不会随机切换。
