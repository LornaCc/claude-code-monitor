...
可从 [Gitea Releases](https://gitea.lan.fasteurai.com/linruyue/claude-code-monitor-desktop/releases) 或 [GitHub Releases](https://github.com/LornaCc/claude-code-monitor/releases) 下载 `CCMonitor-v0.4.1-win-x64.zip`。
...
安装器会停止旧 CC Monitor 实例，将 v0.4.1 安装到 `%LOCALAPPDATA%\Programs\CCMonitor\0.4.1`，重新指向 Claude Code Hooks 和 StatusLine，强制安装并核对 Terminal Bridge 0.4.1，更新开始菜单快捷方式，然后启动新版本。旧版本目录可以保留用于回退，但不会继续运行或被 Hook 引用。
...
- **Terminal Bridge is not running**：确认扩展为 0.4.1，并在每个相关 VS Code 窗口执行 **Developer: Reload Window**。
